...
export class AccountsRepository implements IAccountsRepository {
  ...
  async save(account: Account): Promise<Account> {
    return await this.db.transaction().execute(async (trx) => {
      const result = await trx.insertInto('accounts').values(account).returningAll().executeTakeFirstOrThrow();
  ...
  async getAccountByLogin(login: string): Promise<Account | null> {
    const result = await this.db.selectFrom('accounts').where('login', '=', login).selectAll().executeTakeFirst();
