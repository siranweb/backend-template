import { IRouterNode } from '../../types/router-node.interface';

export class RouterNode implements IRouterNode {
  private static readonly dynamicRoutePartSymbol = Symbol('dynamicRoutePart');
  ...
  private convertRouteParts(routeParts: string[]): (string | symbol)[] {
    ...
    for (const routePart of routeParts) {
      if (routePart.startsWith(':')) {
        parts.push(RouterNode.dynamicRoutePartSymbol);
      } else {
  ...
  private getOrCreateRouteNode(routePart: string | symbol): RouterNode {
    ...
    const isDynamicRoute = routePart === RouterNode.dynamicRoutePartSymbol;
  ...
  private getNodeChildByRoutePart(pathPart: string): RouterNode | null {
    ...
    if (!foundNode) {
      foundNode = this.nodes[RouterNode.dynamicRoutePartSymbol] ?? null;
    }
